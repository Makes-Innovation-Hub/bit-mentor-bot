...
async def test_get_video_length_failure_topic_missing():
    ...
    error_text = "Error: Topic is missing or empty."
    update.message.reply_text.assert_called_once_with(error_text)


@pytest.mark.asyncio
async def test_get_video_length_failure_topic_empty():
    # Setup Mocks
    update = AsyncMock(Update)
    context = AsyncMock(ContextTypes.DEFAULT_TYPE)
    message = AsyncMock(Message)
    ...
    context.user_data = {'topic': ''}  # Topic is empty
    update.message.text = "short"
...
@pytest.mark.asyncio
async def test_get_video_length_failure_length_empty():
    # Setup Mocks
    update = AsyncMock(Update)
    context = AsyncMock(ContextTypes.DEFAULT_TYPE)
    message = AsyncMock(Message)
    ...
    context.user_data = {'topic': 'Python programming'}
    update.message.text = ""  # Invalid length
